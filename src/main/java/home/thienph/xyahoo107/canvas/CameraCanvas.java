...
import home.thienph.xyahoo107.tasks.CameraCaptureTask;
...
public final class CameraCanvas extends Canvas implements CommandListener {
    ...
    private final Command recordCommand = new Command(TextConstant.start(), 4, 1);
    private final Command stopCommand = new Command("Dừng", 4, 1);
    private final Command backCommand = new Command(TextConstant.close(), 2, 1);
    private int cameraMode;

    public static CameraCanvas getInstance() {
        if (instance == null) {
    ...
    public void initializeCamera(int var1) {
    ...
    public static void cleanup(CameraCanvas var0) {
        var0.cleanup();
    }
}
