...
public final class FontRenderer {
    ...
    public static String replaceEmoticons(String string, final boolean b) {
        ...
        for (int i = 0; i < FontRenderer.emoticonCount; ++i) {
            ...
            while ((index = string.toUpperCase().indexOf(FontRenderer.emoticons[i])) != -1) {
                ...
                string = string.substring(0, index) + (char) (i + 30000) + string.substring(index + FontRenderer.emoticons[i].length());
                if (--n > 0) {
                    continue;
                }
                break;
    ...
    public static int getTextWidth(String s) {
        if (FontRenderer.isCustomFontEnabled) {
            ...
            int n = 0;
            char char1;
            for (int n2 = length, index = 0; index < n2 && (char1 = s.charAt(index)) != '\n'; ++index) {
    ...
    public void drawText(final String s, final int n, int n2, final Graphics graphics) {
        if (!FontRenderer.isCustomFontEnabled) {
    ...
    private static String[] splitLineByWords(final String s, final int n) {
        ...
        for (beginIndex = 0, i = s.indexOf(32); i >= 0; i = s.indexOf(32, beginIndex)) {
            ...
            if (getTextWidth(string + substring) >= n) {
        ...
        if (i == -1) {
            ...
            if (getTextWidth(string + substring2) >= n) {
                vector.addElement(string);
                string = "";
            }
            vector.addElement(string + substring2);
        }
